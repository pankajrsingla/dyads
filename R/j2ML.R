j2ML <- function (nets, sender = NULL, receiver = NULL, density =~ 1, reciprocity =~ 1, adapt = NULL, burnin = NULL, center = NULL, separate= NULL, seed = NULL) 
{
  # sampling parameters
  if(!is.null(adapt)){
    Nadapt <- adapt
  ...
  for (i in 1:Nadapt){
    ...
    for (j in 1:Sadapt){ 
      ...
      if ((nnets == 1)){
        ...
        ustar <-  as.vector(c[netnums==k,])  + XC1C2SREmu%*%c(beta[subm], beta[(subs)], beta[(subre)])
        mHCmu <- covHCmu%*%(tXC1C2SREmuvarHCmu%*%ustar)
        munew <- as.vector(Rfast::rmvnorm(1,mHCmu, covHCmu))
        c <- matrix(ustar - XC1C2SREmu%*%munew, ncol=2)
        beta[subC]  <- as.vector(c)
