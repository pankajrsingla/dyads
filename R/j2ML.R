j2ML <- function (nets, sender = NULL, receiver = NULL, density =~ 1, reciprocity =~ 1, adapt = NULL, burnin = NULL, center = NULL, separateSigma = NULL, seed = NULL, densVar = NULL, recVar = NULL) 
  {
  # sampling parameters
  if(!is.null(adapt)){
    neff_min <- adapt
  ...
  sdXm <- c(if (ns>0) {apply(X1, 2, sd)}, if (nre>0) {apply(X2, 2, sd)}, if (densVar == TRUE | nnets == 1) {1}, if (nrandd>0) {rep(1, nnets)}, if ((nd-(as.numeric(nnets>1))*nnets-overalld)>0) {apply(X3[,((as.numeric(nnets>1))*nnets+overalld+1):nd, drop=F], 2, sd)})
  sigmaR <- 1
  ...
  sdXr <- c(if (recVar == TRUE | nnets == 1) {1}, if (nrandr>0) {rep(1, nnets)}, if (nr-((as.numeric(nnets>1))*nnets+overallr)>0) {apply(X4[,((as.numeric(nnets>1))*nnets+overallr+1):nr, drop=F], 2, sd)})
  ...
  ll1C <- lapply(1:nnets, function(k){llj2MLC(yl[[k]], nets[[k]], Xl[[k]], X4l[[k]], c(beta[1:nb], beta[subCnets==k]), g4, Ml[[k]], Myl[[k]], Rl[[k]], cSignl[[k]])})
  ...
  while ((neff_min_obs < neff_min) & (i < Nadapt)){
    ...
    for (j in 1:Sadapt){ 
      ...
      ll2br <- sum(ll2) + sum(lGlm(beta2[c(subb)], sdXm[c(subb)])) + sum(lGlr(g4s[c(subr)], sdXr[c(subr)]))
      ...
      if ((nnets > 1)){
        ...
        r2 <-  g4[subR] + rwR
        g4s[subR] <- r2
        ll2C <- lapply(1:nnets, function(k){llj2MLC(yl[[k]], nets[[k]], Xl[[k]], X4l[[k]], c(beta[1:nb], beta[subCnets==k]), g4s, Ml[[k]], Myl[[k]], Rl[[k]], cSignl[[k]])})
        ll2 <- unlist(lapply(1:nnets, function(k){sum(ll2C[[k]])}))
        if (recVar == TRUE) { 
